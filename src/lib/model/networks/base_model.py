...
from .STSSA.position_encoding import PositionEncodingSine
from .STSSA.stssa import *
def otsu_threshold(data):
    hist = torch.histc(data, bins=256, min=0, max=1)
    hist = hist.float() / hist.sum()
    best_threshold = 0
    best_variance = 0
    for threshold in torch.linspace(0, 1, 256):
        ...
        if w_background == 0 or w_foreground == 0:
            continue
        mean_background = torch.sum(background * torch.arange(int(threshold * 256)).cuda()).cuda() / w_background
        mean_foreground = torch.sum(foreground * torch.arange(int(threshold * 256), 256).cuda()).cuda() / w_foreground
...
class BaseModel(nn.Module):
    def __init__(self, heads, head_convs, num_stacks, last_channel, opt=None):
        ...
        self.pos_encoding = PositionEncodingSine(self.embed_dim,
                                                 temp_bug_fix=True)
    ...
    def forward(self, x, pre_img=None, pre_hm=None, addtional_pre_imgs=None, addtional_pre_hms=None, inference_feats=None):
      ...
      if self.opt.trades:
          feats, embedding, tracking_offset, dis_volume, h_volume_aux, w_volume_aux, mconf \
    ...
    def CVA_MFW(self, feat_list, support_feats):
        ...
        for idx, feat_prev in enumerate(feat_list[1:]):
            c_h, c_w, tracking_offset, mconf = self.CVA(embedding_prime, feat_prev, batch_size, h_c, w_c)
        ...
        return [enhanced_feat], embedding, tracking_offset_output, [h_max_for_loss, w_max_for_loss], h_max_for_loss_aux, w_max_for_loss_aux, mconf
    ...
    def CVA(self, embedding_prime, feat_prev, batch_size, h_c, w_c):
        ...
        embedding_prime, _embedding_prev = self.LoFTR(embedding_prime, _embedding_prev)
        
        from einops import rearrange
        embedding_prime = rearrange(embedding_prime, "n h w c -> n (h w) c")
        _embedding_prev = rearrange(_embedding_prev, "n h w c -> n (h w) c")
        
        _embedding_prev = _embedding_prev.transpose(1,2)

        # Cost Volume Map
        c = torch.matmul(embedding_prime, _embedding_prev)  # (B, H*W/4, H*W/4)
        # sim = torch.einsum("bjc,bck->bjk",embedding_prime,_embedding_prev)

        c_softmax = F.softmax(c,1)*F.softmax(c,2)           # (c,1)当前帧，(c,2)过去帧
        mask = c_softmax > 0.1                              # SNL-0.2、PC-0.1
        mask = mask * (c_softmax == c_softmax.max(dim=1,keepdim=True)[0]) * (c_softmax == c_softmax.max(dim=2,keepdim=True)[0])
        ...
        return c_h, c_w, tracking_offset, [b_ids, i_ids, j_ids]
