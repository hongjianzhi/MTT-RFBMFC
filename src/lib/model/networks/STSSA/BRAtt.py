...
class QKVLinear(nn.Module):
    def __init__(self, dim, qk_dim, bias=True):
        ...
        self.dim = dim
        self.qk_dim = qk_dim
        self.qkv_q = nn.Linear(dim, dim, bias=bias)
        self.qkv_kv = nn.Linear(dim, qk_dim + qk_dim, bias=bias)
    def forward(self, x, source):
        q= self.qkv_q(x)
        kv = self.qkv_kv(source)
        return q, kv
...
class BiLevelRoutingAttention(nn.Module):
    ...
    def forward(self, x, source, x_mask=None, source_mask=None, ret_attn_mask=False):
        ...
        source = rearrange(source, "n (j h) (i w) c -> n (j i) h w c", j=self.n_win, i=self.n_win)
