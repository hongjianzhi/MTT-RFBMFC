...
class Detector(object):
  ...
  def run(self, image_or_path_or_tensor, meta={}, img_id=None):
    ...
    if self.opt.tracking and (len(results)>0):

      public_det = meta['cur_dets'] if self.opt.public_det else None
      results, BoxPreds = self.tracker.step(results, public_det, output["tracking_offset"], output['mconf'], image, pre_processed_images)
      self.pre_images = images
  ...
  def _get_additional_inputs(self, dets, meta, with_hm=True):
    ...
    output_inds = []
    for det in dets:
      self.opt.pre_thresh = self.opt.track_thresh
